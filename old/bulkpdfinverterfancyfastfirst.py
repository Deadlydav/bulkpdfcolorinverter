...
# Function to convert an image to a PDF page with dynamic DPI adjustment
...
# Function to invert colors of a PDF page with dynamic DPI adjustment
def invert_colors(page, temp_image_path, dpi=300, contrast_factor=1.5):
    try:
        if analyze_page_content(page):
            ...
            # Calculate optimal DPI based on image dimensions
            optimal_dpi = max(75, int(300 * (image.width * image.height / (2500 * 2500))**0.5))
            inverted_image.save(temp_image_path, dpi=(optimal_dpi, optimal_dpi))
...
def process_pdf(input_file, output_folder, dpi=300, contrast_factor=1.5):
    try:
        ...
        if os.path.exists(output_file):
            print(f"Skipping existing file: {os.path.basename(input_file)}")
            return
        ...
        with PdfWriter() as pdf_writer:
            for temp_file in temp_files:
                ...
                image_to_pdf(temp_file, temp_pdf_path)
...
# Function to process all PDFs in a directory, prioritizing smaller files first
def process_pdfs_in_directory(input_folder, output_folder, dpi=300, contrast_factor=1.5):
    ...
    files.sort(key=lambda f: os.path.getsize(f[0]))
